"""
How do different models perform on the stacked multi-query associative recall task?
"""
...
WANDB_PROJECT = 'hippogriff-mqar-stacked'
...
def run():
    ...
    seq_len = 64
    train_inputs, train_targets = multiquery_ar(vocab_size=vocab_size, num_examples=num_train_examples, input_seq_len=seq_len, seed=42, power_a=0.01, num_kv_pairs=8, random_non_queries=False)
    valid_inputs, valid_targets = multiquery_ar(vocab_size=vocab_size, num_examples=num_valid_examples, input_seq_len=seq_len, seed=43, power_a=0.01, num_kv_pairs=8, random_non_queries=False)
    ...
    tapes = Tapes(
        ...
        seq_len=seq_len,
        train=Repeat([(input, target) for input, target in zip(train_inputs.to(device).view(num_train_batches, batch_size, seq_len//2, 2),
                                                               train_targets.to(device).view(num_train_batches, batch_size, seq_len//2, 2)[..., 0])]),
        valid=[(input, target) for input, target in zip(valid_inputs.to(device).view(num_valid_batches, batch_size, seq_len//2, 2),
                                                        valid_targets.to(device).view(num_valid_batches, batch_size, seq_len//2, 2)[..., 0])],
    )
...
sweep_configuration = {
    "name": "stacked-mqar8kv_len64+outer+values",
    "method": "grid",
